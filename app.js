...
const workSheetsFromFile = xlsx.parse(`./excel/경주스탬프2.0.xlsx`);

function parser_header(workSheetno){
	var first = workSheetsFromFile[workSheetno].data[2].join();
...
function parser_body(workSheetno,key){
	var jsonData = [];
	for(var j = 3 ; j<workSheetsFromFile[workSheetno].data.length ;j++){
...
function write_json(){
	...
	for(var k = 1 ; k<workSheetsFromFile.length;k++){
		var key = parser_header(k);
		var res_json = parser_body(k,key);
		console.log(key);
		console.log(res_json);

		var file = filepath[k-1];
